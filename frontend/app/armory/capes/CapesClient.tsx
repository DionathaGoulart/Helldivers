...
export default function CapesClient({ initialCapes, initialPasses }: CapesClientProps) {
  ...
  const filteredCapes = useMemo(() => {
    let list = [...initialCapes];

    if (search) {
      const searchLower = search.toLowerCase();
      list = list.filter((c) =>
        (c.name && c.name.toLowerCase().includes(searchLower)) ||
        (c.name_pt_br && c.name_pt_br.toLowerCase().includes(searchLower))
      );
    }
    ...
    if (passField) {
      list = list.filter((c) => (c as any).pass_field === passField);
    }

    return applyCustomOrdering(list, ordering);
  }, [initialCapes, search, source, passField, ordering]);

  useEffect(() => {
    const targetCount = filteredCapes.length;
  ...
  }, [filteredCapes.length, animatedCount]);
