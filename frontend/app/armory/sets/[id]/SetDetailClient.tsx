...
export default function SetDetailClient({ initialSet }: SetDetailClientProps) {
  const router = useRouter();
  const { user } = useAuth();
  const { isPortuguese } = useLanguage();
  const { t } = useTranslation();

  const set = initialSet;
  ...
  const RelationButton = ({
    ...
    return (
      <button
        onClick={() => handleToggleComponentRelation(type, id, relationType)}
        disabled={isUpdating}
