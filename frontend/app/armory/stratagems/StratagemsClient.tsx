...
export default function StratagemsClient({ initialStratagems }: StratagemsClientProps) {
    ...
    }, [initialStratagems, search, department, isPortuguese]);

    // Extract unique departments for filter
    const departments = useMemo(() => {
        const deps = new Set(initialStratagems.map(s => s.department));
        const list = Array.from(deps).map(d => {
            const strat = initialStratagems.find(s => s.department === d);
            return {
                value: d,
                label: strat ? strat.department_display || d : d
            };
        });
