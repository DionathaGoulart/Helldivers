...
import { getCollectionSets, ArmorSet, removeSetRelation } from '@/lib/armory-cached';
...
export default function CollectionPage() {
  ...
  const handleRemoveFromCollection = async (set: ArmorSet) => {
    try {
      ...
      alert(error.response?.data?.detail || error.message || t('collection.removeError'));
    }
  };
      ...
      {authLoading || loading ? (
        <div className="text-center py-12">
          ...
          <p className="mt-4 text-gray-600">
            {authLoading ? t('collection.checkingAuth') : t('collection.loading')}
          </p>
        </div>
      ) : sets.length === 0 ? (
        ...
        <>
          <p className="text-sm text-gray-600 mb-4">
            {t('collection.results', { count: sets.length })}
          </p>
              ...
              <Card key={set.id} className="hover:shadow-lg transition-shadow">
                ...
                <div className="p-6">
                  ...
                  {set.armor_stats && (
                    <div className="grid grid-cols-3 gap-2 mb-4 text-sm">
                      <div className="p-2 bg-gray-50 rounded text-center">
                        <p className="text-gray-600 text-xs">{t('collection.armor')}</p>
                        <p className="font-semibold">{set.armor_stats.armor_display || set.armor_stats.armor}</p>
                      </div>
                      <div className="p-2 bg-gray-50 rounded text-center">
                        <p className="text-gray-600 text-xs">{t('collection.speed')}</p>
                        <p className="font-semibold">{set.armor_stats.speed_display || set.armor_stats.speed}</p>
                      </div>
                      <div className="p-2 bg-gray-50 rounded text-center">
                        <p className="text-gray-600 text-xs">{t('collection.stamina')}</p>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <span className="text-lg font-semibold text-gray-700">{t('collection.totalCost')}</span>
